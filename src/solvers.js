...
window.countNQueensSolutions = function(n) {
  //Martin Richard's bitwise solution

  var solutionCount = 0;
  var all = Math.pow(2, n) - 1;
 
  var solveBitwise = function (ld, col, rd) {
    if (col === all && n > 0) {
      solutionCount++;
      return;
    }
    ...
    while (poss) {
      var bit = poss & -poss;
      poss -= bit;
      solveBitwise((ld | bit) << 1, (col | bit), (rd | bit) >> 1);
    }
  };
